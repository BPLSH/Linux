...
- sudo fdisk -l : 전체 디스크의 파티션 정보 출력  
  
### 파일 시스템 생성하기 ###  
**파일 시스템 생성 명령**  
리눅스에서 파일 시스템을 만들 때 사용하는 명령은 mkfs와 mke2fs가 있다.  
**mkfs**
...
- i inode당 바이트 수 : inode당 바이트 수를 지정한다(기본 값 4096B)  
- m 예약 블록 퍼센트 : 슈퍼유저에게 예약해둘 블록의 퍼센트를 지정한다(기본값 5%)  
  
### 디스크 마운트하기 ###  
**마운트 포인트 준비하기**  
/mnt 디렉터리에 마운트 포인트로 사용할 hdd1, hdd2, hdd3를 생성  
sudo mkdir /mnt/hdd1 sudo mkdir /mnt/hdd2 sudo mkdir /mnt/hdd3  
...
/etc의 hosts 파일을 /mnt/hdd1으로 복사  
umount하면 파일 시스템과의 연결이 해제되어 파일을 볼 수가 없다.  
  
### 여러 디스크를 하나처럼 사용하기 ###  
LVM은 파티션을 효율적으로 사용할 수 있도록 해주는 관리 도구로, 파티션의 용량이 부족할 때 다른 파티션으로 연장하여 사용가능  
독립적으로 구성된 디스크 파티션을 하나로 연결하여 한 파티션처럼 사용할 수 있도록 해준다.  
PV : /dev/sdb1과 같은 실제 하드디스크의 파티션을 의미한다.  
VG : 여러개의 PV를 그룹으로 묶은 것을 의미한다.  
...
### LVM 생성 과정 ###  
기존 파일 시스템의 종류 변경 - PV 생성 - VG 생성 - VG 활성화 - LV 생성 - LV에 파일 시스템 생성 - LV 마운트  
